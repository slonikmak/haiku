...
function displayRandomCard() {
  ...
  if (authorEl) authorEl.innerText = "Автор: " + card.author;
  if (japaneseEl) japaneseEl.innerText = card.japanese;
  if (englishEl) englishEl.innerText = card.english;
  if (russianEl) russianEl.innerText = card.russian;
}

function init() {
  const appEl = document.getElementById("app");
  if (appEl) {
    appEl.onclick = displayRandomCard;
  }
  displayRandomCard();
}
